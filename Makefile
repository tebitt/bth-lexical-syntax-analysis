...
clean:
		rm -f parser.tab.* lex.yy.c* compiler stack.hh position.hh location.hh tree.dot tree.pdf symbol_table.dot symbol_table.pdf ir.dot ir.pdf bytecode.txt bytecode.class
		rm -R compiler.dSYM 
st: 
